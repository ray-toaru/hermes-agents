...
| Structured command evidence and dispatch | ADR 0009 | Audit command strings are recorded-only evidence; structured command evidence contract exists, but no command runner exists. | Add reviewed allowlisted command dispatch through argv-only execution, path/env/timeout/redaction policies, and tests. |
